...
export function llmsTxt(): string {
  return [
    ...
    '',
    'Personal site of one independent engineer. The home page - profile, projects,',
    'experience, contact - is mirrored as plain Markdown. The secondary pages',
    '(about, contact, API reference, and the two legal pages) are HTML only; the',
    'legal text deliberately so, to keep exactly one authoritative wording. No',
    'JavaScript is required to read any of it.',
    '',
    '## When to use this',
    '',
    ...
    '  It needs no API key, is limited to 10 requests per hour, and returns JSON',
    '  errors carrying a stable `code` and a `resolution` hint. When the site is',
    '  running with Friendly Captcha enabled, a browser-solved `frcCaptchaResponse`',
    '  token is also required and a call without one answers 400 `captcha_required`;',
    `  email ${CONTROLLER.email} directly if that happens.`,
    '',
    ...
    '## Full text',
    '',
    `- [llms-full.txt](${url}/llms-full.txt): the home page plus the operator and legal details, in one file.`,
    '',
    '## Machine-readable',
    '',
